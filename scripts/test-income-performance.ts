...
// Primary plan metric: keep spending and taxes deducted, remove only funding.
{
  ...
  assert.equal(r.totalGrowth, 5000, 'accumulation growth includes new contributions')
  ...
  assert.equal(calculate(base, [], 'a', '2026-08').totalGrowth, 2500)
  ...
  assert.equal(equityGrowth([{ ...rows[0], flowsAvailable: false }], [], accounts).totalGrowth, 2500, 'missing cash-flow detail must not hide observed accumulation progress')
  assert.equal(equityGrowth([{ ...rows[0], openingEquity: undefined }], [], accounts).value, null)
  const balances = [{ ...statement('2026-08', 10000, 5000, 0), withdrawals: -5000 }, { ...statement('2026-08', 2000, 11000, 8000, '2222'), withdrawals: 0 }]
  const transfer = [
  ...
  assert.equal(unknown.value, null, 'unreconciled funding is not a confirmed loss headline')
  assert.equal(unknown.provisionalValue, -4000)
  assert.equal(unknown.ending - unknown.beginning, 4000, 'total equity growth still counts contributions')
  assert.equal(unknown.fundingNeedsReview, true)
  assert.equal(unknown.totalGrowth, 4000)
  assert.equal(unknown.estimated, true)
  assert.equal(unknown.funding[1].deposits, 8000)
  assert.equal(unknown.funding[1].ledgerDeposits, 0)
  assert.match(unknown.funding[1].issues.join(' '), /Deposit transactions do not reconcile/)
  ...
  const classifiedTransfers = transfer.map((t) => t.id === 'paycheck' ? t : { ...t, type: 'Transfer' as const })
  const repaired = calculate(balances, classifiedTransfers, 'all')
  assert.equal(repaired.value, 1000, 'explicit Transfer legs count as internal without reclassifying saved records')
  assert.equal(repaired.fundingNeedsReview, false)
  assert.equal(repaired.transferMatches.length, 1)
  assert.deepEqual(repaired.transferMatches[0], { incoming: classifiedTransfers[1], outgoing: classifiedTransfers[0] })
  const noDoubleSubtract = statementHistory(balances, accounts, 'all').map((r) => ({ ...r, deposits: 3000, statements: r.statements.map((s) => ({ ...s, source: 'Automatic snapshot' as const, deposits: s.accountMask === '2222' ? 3000 : 0, withdrawals: 0 })) }))
  const snapshotTransfers = equityGrowth(noDoubleSubtract, classifiedTransfers, accounts, '', '2026-09-22')
  assert.equal(snapshotTransfers.value, 1000, 'snapshot already excludes Transfer cash from deposits; do not subtract it twice')
  assert.equal(snapshotTransfers.internalDeposits, 0)
  const bill = calculate(balances, transfer.map((t) => t.id === 'out' ? { ...t, type: 'Bill Payment' as const } : t), 'all')
  assert.equal(bill.internalDeposits, 0, 'bill payment matching a deposit amount is not transfer evidence')
  assert.equal(bill.value, -4000)
  const ambiguous = calculate(balances, [...transfer, { ...transfer[1], id: 'another-deposit' }], 'all')
  assert.equal(ambiguous.value, null)
  assert.equal(ambiguous.transferMatches.length, 0, 'one withdrawal cannot choose arbitrarily between identical deposits')
  assert.match(ambiguous.funding.flatMap((f) => f.issues).join(' '), /Multiple equal cash movements/)
  const unmatched = calculate(balances, classifiedTransfers.filter((t) => t.id !== 'out'), 'all')
  assert.equal(unmatched.value, null)
  assert.match(unmatched.funding.flatMap((f) => f.issues).join(' '), /Unmatched transfers/)
  const unclassified = calculate([base[1]], [txn('Other', 1000, '2026-08-25')])
  assert.equal(unclassified.value, null)
  assert.match(unclassified.funding[0].issues.join(' '), /Unclassified activity/)
  // Fully invest $5,000 wages: assets go from $10,000 to $15,000. Pay
  // $2,000 bills + $100 interest on margin: debt $2,100, equity $12,900.
  const accumulation = calculate([{ ...statement('2026-08', 10000, 15000 - 2100, 5000), withdrawals: -2000, expenses: -100 }])
  assert.equal(accumulation.totalGrowth, 2900, 'wages count; margin-funded bills and interest remain deducted once')
  assert.equal(accumulation.value, -2100, 'secondary diagnostic removes wages but does not define accumulation success')
  assert.equal(accumulation.months[0].totalGrowth, 2900)
  const spendingOnly = calculate([{ ...statement('2026-08', 10000, 7900, 0), withdrawals: -2000, expenses: -100 }])
  assert.equal(spendingOnly.totalGrowth, -2100)
  const malformedFunding = calculate([{ ...statement('2026-08', 10000, 12900, NaN) }])
  assert.equal(malformedFunding.totalGrowth, 2900)
  assert.equal(malformedFunding.value, null)
