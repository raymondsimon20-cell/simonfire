...
export function EquityBridge({ close, transactions, positions, accounts, scope, chart = false }: {
  ...
  const knownValue = (index: number) => [1, 2, 3].includes(index) || (index === 5 && currentClosing) || close.historyAvailable
  const accountName = (id: string) => accounts.find((a) => a.id === id)?.name ?? id
  const total = rows.reduce((n, r) => n + r.value, 0)
  return <>
    {chart && close.historyAvailable && <Waterfall steps={close.bridge} height={300} onStepClick={setSelected} />}
    {!close.historyAvailable && <p className="mt-2 text-xs text-muted">Historical equity snapshots are unavailable. Transaction totals are shown; opening equity, market movement, and historical closing equity are not inferred.</p>}
