...
import { dividendLookupStatus } from '../lib/dividend-lookup-status'
    ...
    <div>
      ...
      <div className="card mb-6 p-5">
        ...
        <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead><tr className="border-b border-border-soft text-left text-xs text-muted"><th className="py-2">Account</th><th>Last requested history</th><th>Returned positions / transactions</th><th>Dividends missing tickers</th></tr></thead><tbody>{data.accounts.map((account) => {
          ...
          const lookup = dividendLookupStatus(data.transactions, account.id)
          return <tr key={account.id} className="border-b border-border-soft"><td className="py-3 font-medium">{account.name}</td><td className="text-xs text-muted">{coverage ? <>{coverage.from} – {coverage.to}<div className="mt-1 text-faint">{relTime(coverage.syncedAt)} · {coverage.method}</div></> : 'Not returned by the latest sync; saved data retained'}</td><td className="num">{coverage ? `${coverage.positionCount} / ${coverage.transactionCount}` : 'Sync to verify'}</td><td className={clsx('num py-3', lookup.missing ? 'text-[#f0a94a]' : 'text-pos')}>{lookup.missing}<div className="mt-1 text-xs text-muted">{lookup.missing === 0 ? 'All dividend tickers assigned' : <>{lookup.deferred > 0 && <div>{lookup.deferred} queued for the next sync</div>}{lookup.unavailable > 0 && <div>{lookup.unavailable} unavailable · retry sync</div>}{lookup.unmatched > 0 && <div>{lookup.unmatched} checked · Schwab returned no matching ticker</div>}{lookup.missing > lookup.deferred + lookup.unavailable + lookup.unmatched && <div>Review unassigned payments in Transactions</div>}</>}</div></td></tr>
        })}</tbody></table></div>
