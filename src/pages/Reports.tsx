...
export default function Reports() {
  ...
  const history = statementHistory(data.historicalBalances ?? [], accounts, scope)
  ...
  return <div className="print-report"><PageHeader title="Monthly Portfolio Statement" subtitle="Equity, income, spending, realized results, margin, and protection" right={<><select aria-label="Report month" value={month} onChange={(event) => setMonth(event.target.value)} className="rounded-xl border border-border bg-surface-2 px-3 py-2 text-sm">{months.map((value) => <option key={value} value={value}>{monthLabel(value)}</option>)}</select><Button onClick={() => window.print()}><Printer size={14}/> Print / PDF</Button><Button onClick={exportReport}><Download size={14}/> Export</Button></>}/>
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-2"><div className="card p-5"><h2 className="text-lg font-semibold">Equity bridge</h2><EquityBridge key={`${scope}-${month}`} close={report.close} transactions={transactions} positions={positions} accounts={accounts} scope={scope}/></div><div className="card p-5"><h2 className="text-lg font-semibold">What changed and why?</h2><div className="mt-4 space-y-3 text-sm"><Explanation label="Dividend income" value={dividendDelta} text={`${usd(Math.abs(dividendDelta))} ${dividendDelta >= 0 ? 'more' : 'less'} than the prior month, based on received dividend transactions.`}/><Explanation label="Operating expenses" value={-expenseDelta} text={`${usd(Math.abs(expenseDelta))} ${expenseDelta >= 0 ? 'higher' : 'lower'} than the prior month, including fees and margin interest.`}/><p className="text-xs text-muted">{report.close.historyAvailable ? 'Equity change and bridge use imported statement balances. Income and spending details use available transactions.' : 'Historical equity change is unavailable until beginning and ending balances are recorded. Contributions are not investment profit.'}{report.close.statement && !report.close.statement.complete && ' Partial account coverage: the equity totals include only imported accounts.'}</p></div></div></div>
